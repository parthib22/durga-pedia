...
export async function POST(request: any) {
  try {
      ...
      const apiCalls = types.map((typeval) => {
        const apiurl = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?radius=1500&location=${lat},${lng}&keyword=${typeval.kword}&type=${typeval.type}&key=${googleMapsApiKey}`; 
     console.log(apiurl);
